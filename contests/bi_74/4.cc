...
int min_white(string_view floor,
  int idx,
  int num_carpets,
  int len,
  vector<int> const &num_white,
  vector<vector<int>> &dp) {
  if (idx >= size(floor)) return 0;
  if (num_carpets == 0) return num_white[idx];
  if (dp[idx][num_carpets] != -1) { return dp[idx][num_carpets]; }
  if (floor[idx] == '1') {
    return dp[idx][num_carpets] = min(
             1 + min_white(floor, idx + 1, num_carpets, len, num_white, dp),
             min_white(floor, idx + len, num_carpets - 1, len, num_white, dp));
  } else {
    return dp[idx][num_carpets] =
             min_white(floor, idx + 1, num_carpets, len, num_white, dp);
  }
}
